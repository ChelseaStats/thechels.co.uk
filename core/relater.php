<?php
	/*
	Plugin Name: CFC Related Articles by Category
	Description: WordPress plugin for related posts with a category ordered by most recent. It's small. It's fast.
	Version: 11.0.0
	Author: ChelseaStats
	...
	*/
	...
	class RPBC {
		...
		$posts = $GLOBALS['wpdb']->get_results(
				...
				sprintf("SELECT DISTINCT object_id as ID, post_title FROM {$GLOBALS['wpdb']->term_relationships} r, {$GLOBALS['wpdb']->term_taxonomy} t, {$GLOBALS['wpdb']->posts} p
						...
						AND r.object_id = $id %s) AND r.term_taxonomy_id = t.term_taxonomy_id AND p.post_status = 'publish'
						AND p.ID = r.object_id AND object_id <> $id %s %s %s",
				...
				if (!empty($params['image']) && !(isset($params['hidden']) && $params['hidden'] == 'image')) {
					...
					if (empty($thumb)) {
						...
						if (!empty($params['default'])) {
							...
							$image = sprintf('<img src="%s" class="rpbc_default" alt="%2$s" title="%2$s" />',esc_url($params['default']),$title);
						}

					} else {

						$image = preg_replace('#alt=([\'"]).*?([\'"]) title=([\'"]).*?([\'"])#si','alt=${1}' .$title. '${2} title=${3}' .$title. '${4}',$thumb);
					}
				}
				$output .= sprintf('%s<a href="%s" title="%s"%s>%s%s%s%s</a>%s',$before,esc_url(get_permalink($post->ID)),$title,$rel,$image,$inside,$hidden,$outside,$after);
			}
		} else {
	...
	add_action('related_posts_by_category','related_posts_by_category',10,2);
