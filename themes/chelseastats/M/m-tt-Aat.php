<?php /* Template Name: # m-T5 away at*/ ?>
<?php get_header(); ?>
<?php if (current_user_can('level_10')) : ?>
	<div id="content">
		<div id="contentleft">
			...
			<p>top 5 away teams by number of wins at the home of team chosen</p>
			<?php
				...
				if (isset($team) && $team !='')  {
					...
					$pdo = new pdodb();
					$pdo->query('SELECT F_AWAY N, count(*) as V FROM all_results WHERE F_AGOALS > F_HGOALS AND F_HOME =:team
						         GROUP BY F_AWAY ORDER BY count(*) DESC LIMIT 0,5');
					$pdo->bind(':team',$team);
					$rows = $pdo->rows();
					foreach($rows as $row) {
						$ns[] = $row['N'];
					...
					$team = $go->_V($team);
					$message ="#Stats Top 5 PL winners away at ".$team.": ".$nv;
