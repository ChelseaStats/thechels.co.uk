<?php /*  Template Name: # XML feed EPL */
	...
	$feed		    = "The London League by ChelseaStats";
	$desc		    = "The London League results and tables";
	$title_this 	= "London League Table ({$rss->getSeasonalDate(date('Y-m-d'))})";
	$title_all	    = "London League Table (all time)";
	$type           = "london";

	$query_key      = "SELECT F_DATE as D, F_HOME as H, F_AWAY as A FROM all_results
					    WHERE ((F_HOME in ('WEST_HAM','QPR','FULHAM','BRENTFORD','CHARLTON', 'C_PALACE','MILLWALL','ARSENAL','SPURS','CHELSEA','WIMBLEDON'))
	...
	$query_this     = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A,SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_LDN_this GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";
	...
	$query_all      = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A, SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_LDN GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";
