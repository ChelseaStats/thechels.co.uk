<?php /*  Template Name: # XML feed two */
	header('Content-Type: text/xml; charset=utf-8');
	$rss            = new utility();
	$thisYear       = date('Y-m-d');
	$url 		    = "http://thechels.co.uk/rss-feeds/";
	$feed		    = "FAWSL 2 by ChelseaStats";
	$desc		    = "The FAWSL 2 results and tables";
	$title_this     = "FA WSL 2 Table ({$thisYear})";
	$title_all	    = "All Time FA WSL 2 Table";
	$type           = "wsltwo";

	$query_key      = "SELECT F_DATE as D, F_HOME as H, F_AWAY as A
						FROM all_results_wsl_two ORDER BY F_DATE DESC LIMIT 1";
	...
	$query_results  = "SELECT F_DATE as D, F_HOME as H_TEAM, F_HGOALS as HG, F_AGOALS as AG, F_AWAY as A_TEAM
			 		  	FROM all_results_wsl_two ORDER BY F_DATE DESC LIMIT 10";
