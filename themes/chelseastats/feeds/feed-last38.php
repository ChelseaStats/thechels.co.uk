<?php /*  Template Name: # XML feed EPL */
	...
	$feed		    = "Last 38 League by ChelseaStats";
	$desc		    = "The Last 38 League results and tables";
	$title_this 	= "Last 38 League Table ({$rss->getSeasonalDate(date('Y-m-d'))})";
	$title_all	    = "Last 38 League Table (all time)";
	$type           = "last38";
	...
	$query_all      = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A, SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_last38 GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";
