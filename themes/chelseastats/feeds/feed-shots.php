<?php /*  Template Name: # XML feed Shotd */
	...
	$feed		    = "Premier League Shots by ChelseaStats";
	$desc		    = "Premier League Shots results and tables";
	$title_all 	= "Premier League Shots Table ({$rss->getSeasonalDate(date('Y-m-d'))})";
	$title_this	    = "Premier League Shots Table";
	$type           = "shots";
	...
	$query_all     = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A,SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_Shots_this GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";

	$query_results  = "SELECT F_DATE as D, F_HOME as H_TEAM, H_SHOTS as HG, A_SHOTS as AG, F_AWAY as A_TEAM
