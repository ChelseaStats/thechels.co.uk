<?php /*  Template Name: # XML feed t13 */
	header('Content-Type: text/xml; charset=utf-8');
	$rss            = new utility();
	$url 		    = "http://thechels.co.uk/rss-feeds/";
	$feed		    = "Threatened 13 League by ChelseaStats";
	$desc		    = "The Threatened 13 League results and tables";
	$title_this 	= "Threatened 13 League Table ({$rss->getSeasonalDate(date('Y-m-d'))})";
	$title_all	    = "Threatened 13 League Table (all time)";
	$type           = "t13";

	$query_key      = "SELECT F_DATE as D, F_HOME as H, F_AWAY as A FROM all_results
		    			WHERE ((F_HOME not in ('MAN_CITY','ARSENAL','SPURS','CHELSEA','MAN_UTD','LIVERPOOL','EVERTON'))
	            		AND (F_AWAY not in ('MAN_CITY','ARSENAL','SPURS','CHELSEA','MAN_UTD','LIVERPOOL','EVERTON')))
	...
	$query_this     = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A,SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_T13_this GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";
	...
	$query_all      = "SELECT Team, SUM(PLD) PLD, SUM(W) W, SUM(D) D, SUM(L) L, SUM(FS) FS, SUM(CS) CS, SUM(BTTS) BTTS, SUM(F) F, SUM(A) A, SUM(GD) GD, round(sum(PTS)/sum(PLD),3) PPG, SUM(PTS) PTS
				   		FROM 0V_base_T13 GROUP BY Team ORDER BY PTS DESC, GD DESC, PPG DESC, Team DESC";
